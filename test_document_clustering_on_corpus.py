...
class TestDocumentClusteringOnCorpus(unittest.TestCase):
    ...
    def test_clustering(self):
        ...
        cluster = FactoryLoader.load(
            StepsName.CLUSTERING, str(self.test_path.resolve()), self.process_name, data, emb
        )
        graph = FactoryLoader.load(
            StepsName.GRAPH, str(self.test_path.resolve()), self.process_name
        )
        word_embedding_clustering = cluster_functions.WordEmbeddingClustering.with_objects(sentence_embedding_obj=emb)
        ...
        _ari = word_embedding_clustering.ari_score(
            embeddings_cluster_obj=cg_clustering,
