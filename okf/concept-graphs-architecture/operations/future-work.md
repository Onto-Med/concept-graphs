...
4. **Expand domain-profile support if non-medical SONs matter.**
   ...
   **Deferred TODO: profile-owned relation definitions.** The current profile metadata API exposes relations derived from Python/global relation definitions and filtered by profile categories; this is sufficient for immediate TOP integration. Profiles can already provide UI-only `relation_labels`, but relation definitions themselves are not yet profile-owned. Ultimately, query-expansion profiles should support explicit profile-owned relation definitions in `conf/query-expansion/profiles/*.yml`, similar to profile-owned category vocabularies. Profile relations should define a stable `id`, display `label`, `description`, and source/target category constraints together. When implemented, `medical_de` and `medical_en` should explicitly declare the current medical relation set, and API profile relation metadata should be sourced from profile YAML rather than hardcoded Python defaults. Python relation definitions, if retained, should become generic schema/helpers or optional templates, not the source of truth; an optional `label` field on the helper `RelationDefinition` model may still be useful for template/default definitions. Tests should include a non-medical custom profile, for example an architecture profile with custom relations.

5. **Move query-expansion runtime activation toward a RAG-like model.**
   Today, callers provide LLM model/provider/runtime options on each `/query-expansion` request. Long term, query expansion should behave more like RAG activation/status: Concept Graphs should own server-side LLM/runtime activation and default configuration for query expansion, expose active/default status/configuration as needed, and use that active configuration for ordinary expansion requests. Downstream clients such as TOP should normally send only semantic/user choices and data-source-owned profile/relation constraints; per-request LLM model/provider/base URL/API-key injection should be reserved for optional admin/debug overrides rather than required integration behavior. This is an architecture direction only and must not introduce TOP-specific retrieval/query behavior into Concept Graphs.

6. **Improve grounding sources.**
