...
def data_processing_medical(
    cache_path: Path,
    name_prefix: str,
    labels: Union[dict, Callable],
    n_process: int = 1,
    use_lemma: bool = False,
    prepend_root: bool = False,
    head_only: bool = False,
    sub_path: Optional[list] = None,
    ...
    data_functions.DataProcessingFactory.create(
        ...
        labels=labels,
        sub_paths=(
            ["Schulz-Arztbriefe", "life-100-part01"] if sub_path is None else sub_path
        ),
...
def data_processing_1000PA(
    ...
    head_only: bool = False,
    suffix: Optional[str] = None,
    sample_size: Union[float, int] = 1.0,
    ...
    def _base_data(data_path):
        for d in data_path.glob("*.txt"):
            yield {
                "name": d.stem,
                "content": _clean_1000pa(d.read_text(encoding="utf-8")),
    ...
    _cats = random.sample(
        [i[0] for i in _label_count.items() if i[1] > min_cat_num],
        min(category_count, len(_max_labels)),
    )

    _sorted_cats = [
        x
        for x in sorted(_label_count.items(), key=lambda x: x[1], reverse=False)
        if x[0] in _cats
    ]
...
def data_processing_ng20(
    ...
    _data = fetch_20newsgroups(
        subset=subset, categories=categories, remove=clean, random_state=random_state
    )
    ...
    _base_data = (
        {"name": n.split(os.sep)[-1], "content": d, "label": n.split(os.sep)[-2]}
...
def data_processing_ohsumed(
    ...
    _base_data = [
        {"name": p.name, "content": p.read_text(), "label": p.parent.name}
        for p in _data_path
        if p.is_file()
    ]
...
def data_processing_medline(
    ...
    if merge_consecutive:
        ...
        for _cat in Path("../../data/medline_sample").glob("*"):
            ...
            _base_data += [
                {
                    "name": f"{p1.name}_{p2.name}",
                    "content": f"{p1.read_text()} {p2.read_text()}",
                    "label": p1.parent.name,
                }
                for p1, p2 in pairwise(_files)
            ]
    else:
    ...
    data_functions.DataProcessingFactory.create(
        ...
        # spacy.load("../../ext_models/en_core_web_trf-3.2.0/en_core_web_trf/en_core_web_trf-3.2.0/"),
        base_data=(
            d for d in _base_data if d["label"] in _classes
        ),  # itertools.islice(_base_data, _data_len),
        cache_path=cache_path,
        cache_name=f"{name_prefix}_data-processed",
        n_process=n_process,
        use_lemma=use_lemma,
...
def data_processing_mtsamples(
    ...
    for _c in _classes:
        ...
        _sample_data = _data_points.sample(
            min(
                min_class_samples + random.randint(0, sample_spread),
                len(_data_points.index),
            )
        )
        for _, _row in _sample_data.iterrows():
            ...
            _base_data.append(
                {
                    "name": str(_row[0]),
                    "content": _row[_text_col],
                    "label": _row[_class_col],
                }
            )
    ...
    embedding_functions.SentenceEmbeddingsFactory.create(
        data_obj=data_functions.DataProcessingFactory.load(
            data_obj_path=(
                cache_path
                / Path(
                    f"{name_prefix}_data-processed{('_' + suffix) if suffix is not None else ''}"
                )
            )
        ),
        cache_path=cache_path,
...
def phrase_clustering_various(
    ...
    **kwargs,
    ...
    if ref_data_str is None:
        data_obj = cache_path / Path(
            f"{name_prefix}_data-processed{('_' + suffix) if suffix is not None else ''}"
        )
    else:
        data_obj = cache_path / Path(
            f"{ref_data_str}_data-processed{('_' + suffix) if suffix is not None else ''}"
        )
    cluster_functions.PhraseClusterFactory.create(
        sentence_embeddings=embedding_functions.SentenceEmbeddingsFactory.load(
            ...
            embeddings_obj_path=(
                cache_path
                / Path(
                    f"{name_prefix}_phrase-embeddings{('_' + suffix) if suffix is not None else ''}"
                )
            ),
            view_from_topics=view_from_topics,
        ),
        ...
        scaling_n_neighbors=scaling_n_neighbors,
        scaling_min_dist=scaling_min_dist,
        scaling_n_components=scaling_n_components,
        scaling_metric="euclidean",
        scaling_random_state=42,
        kelbow_k=(10, 100),
        kelbow_show=False,
...
if __name__ == "__main__":
    ...
    _components = (
        ["data-processing", "phrase-embedding", "clustering"]
        if len(sys.argv) <= 2
        else [a.strip() for a in sys.argv[2].split(",")]
    )
    ...
    if _pipe == "medical":
        ...
        med_labels = {
            k: v[0] for k, v in load_pickle(_cache_path / Path("file2class")).items()
        }
        if "data-processing" in _components:
            data_processing_medical(
                ...
                n_process=4,
                head_only=False,
                sub_path=["life-100-part01"],
                suffix=_suffix,
            )
        time.sleep(1.0)
        if "phrase-embedding" in _components:
            phrase_embedding_german(
                _cache_path,
                name_prefix=_name_prefix,
                down_scale=False,
                n_process=4,
        ...
        if "clustering" in _components:
            phrase_clustering_various(
                _cache_path,
                name_prefix=_name_prefix,
                cluster_by_down_scale=True,
                scaling_n_neighbors=0.05,
                suffix=_suffix,
            )
        ...
        if "data-processing" in _components:
            data_processing_ng20(
                _cache_path, name_prefix=_name_prefix, n_process=4, subset="all"
            )
        time.sleep(1.0)
        if "phrase-embedding" in _components:
            phrase_embedding_english(_cache_path, name_prefix=_name_prefix)
        ...
        if "data-processing" in _components:
            data_processing_articles(
                _cache_path,
                name_prefix=_name_prefix,
                labels=lambda x: x.split("-")[0],
                n_process=4,
            )
        time.sleep(1.0)
        if "phrase-embedding" in _components:
        ...
        _name_prefix = "ncbi"
        if "data-processing" in _components:
            data_processing_ncbi(
                _cache_path,
                name_prefix=_name_prefix,
                labels=lambda x: x.split("_")[0],
                n_process=4,
            )
        time.sleep(1.0)
        if "phrase-embedding" in _components:
        ...
        if "clustering" in _components:
            phrase_clustering_various(
                _cache_path, name_prefix=_name_prefix, kelbow_metric="distortion"
            )
