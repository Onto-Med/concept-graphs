...
metrics = {
    "KMeans": {"cluster_obj": KMeans, "kwargs": {}, "kwargs_restr": {}},
    "Agglomerative": {
...
def metrics_generator():
    for _alg_name, _metr_dict in metrics.items():
        ...
        if len(_kwargs_dict) == 0:
            ...
            continue
        _kwargs_combs = [
            product_iter([_kwargs_key], _kwargs_value)
        ...
        for _comb in product_iter(*_kwargs_combs):
            ...
            if (
                _metrics["linkage"] == "ward"
                and not _metrics["affinity"] == "euclidean"
            ):  # Todo: use kwargs_restr
                continue
...
def create_clusterings(
    embedding_clustering_factory, algorithm="concept_graph", stop_words=None, **kwargs
) -> Tuple[
    Union[
        cluster_functions.WordEmbeddingClustering._WEClustering,
        cluster_functions.WordEmbeddingClustering._ConceptGraphClustering,
    ],
    dict,
]:
    ...
    if algorithm.lower() == "we_clustering":
        clustering = embedding_clustering_factory.create_we_clustering(
            use_lemma=kwargs.get("use_lemma", False)
        )
        ...
        _kwargs = {
            "cluster_distance": 0.6,  # for each cluster center found, how far away can the corresponding phrases be at max (cosine: 0 - 1; the higher the more restricitve)
            "cluster_min_size": 4,  # how many phrases shall the cluster have at min
            "graph_cosine_weight": 0.85,  # how much influence shall the cosine distance between phrases have, when calculating edge weights for the graph (0 - 1)
            "graph_merge_threshold": 0.95,  # at what string similarity shall phrases be merged to one phrase (0 - 1; 1 is the same string)
            "graph_weight_cut_off": 0.6,  # below what weight values shall edges be removed between phrases (0 - 1)
            "graph_unroll": True,  # whether the graph shall be transformed to tree-like
            "graph_simplify": 0.5,  # proportion on how many edges shall be cut from the graph (0 - 1)
...
def calculate_scores(
    embedding_clustering_factory, clustering_alg_obj, metrics_iterator: Iterable
):
    ...
    for alg_name, alg_impl, alg_metrics in metrics_iterator:
        ...
        if len(alg_metrics) != 0:
            _key = "-".join(
                [v for _, v in sorted(alg_metrics.items(), key=lambda item: item[0])]
            )
        _ari = embedding_clustering_factory.ari_score(
            embeddings_cluster_obj=clustering_alg_obj,
            clustering_obj=alg_impl,
            min_concepts=min_concepts,
            **{f"cluster_{k}": v for k, v in alg_metrics.items()},
        )

        _purity = embedding_clustering_factory.purity_score(
            embeddings_cluster_obj=clustering_alg_obj,
            clustering_obj=alg_impl,
            min_concepts=min_concepts,
            **{f"cluster_{k}": v for k, v in alg_metrics.items()},
...
if __name__ == "__main__":
    ...
    exclusion_ids = (
        ...
        if len(sys.argv) <= 2
            ...
            [int(i.strip()) for i in sys.argv[2].split(",")]
            if sys.argv[2] != "None"
            else []
        )
    )
    fix = (
        {"concept_graph": False, "we_clustering": True}
        if len(sys.argv) <= 3
        else eval(sys.argv[3])
    )
    ...
    for alg in ["concept_graph", "we_clustering"]:
        alg_obj_path = pathlib.Path(
            cache_path
            / pathlib.Path(
                f"{alg}_{prefix}{('_' + suffix) if suffix is not None else ''}.pickle"
            )
        )
        ...
        if alg_obj_path.exists() and fix[alg]:
            ...
            _impl, _kwargs = create_clusterings(
                embedding_clustering_factory=factory, algorithm=alg
            )
    ...
    with (
        cache_path
        / pathlib.Path(
            f"{prefix}_scores_dict{('_' + suffix) if suffix is not None else ''}.pickle"
        )
    ).open(mode="wb") as dump:
