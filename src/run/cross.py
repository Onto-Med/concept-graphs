...
from test import load_embedding_clustering_factory, create_clusterings, calculate_scores, metrics_generator

if __name__ == "__main__":
    params_dict = {
        # "cluster_distance": [.45, .5, .55],
        "cluster_distance": [.5, .6, .7],
        ...
        "graph_distance_cutoff": [.75],
    }

    prefix = "ng20-small" if len(sys.argv) <= 1 else sys.argv[1]
    exclusion_ids = [] if len(sys.argv) <= 2 else [int(i) for i in sys.argv[2].split(",")]
    ...
    logging.info(f"Loading data, embedding & cluster objects for '{prefix}{('_' + suffix) if suffix is not None else ''}' and exclusion ids '{exclusion_ids}' ...")
    ...
    concept_clustering, _ = create_clusterings(embedding_clustering_factory=factory, algorithm='concept_graph')
    ...
    for i, _params in enumerate(itertools.product(*_params_value)):
        ...
        _key_string = "+".join([f"{str(i)}:{str(j)}" for i, j in zip(_params_name, _params)])
    ...
    pickle.dump(scores_dict, (cache_path / f"cross-concept_graph-{prefix}-scores{('_' + suffix) if suffix is not None else ''}.pickle").open('wb'))
