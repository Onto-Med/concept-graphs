...
class MarqoChunkEmbeddingStore(ChunkEmbeddingStore):
    ...
    def _has_index(self):
        return any(
            [
                (self._index_name == d["indexName"])
                for d in self._client.get_indexes()["results"]
            ]
        )
    ...
    def reset_index(self, with_settings: Optional[dict[str, Any]] = None) -> None:
        ...
        self._client.create_index(index_name=self._index_name, settings_dict=_settings)
    ...
    def get_chunks(
        ...
        filter_by = (
            None if ((filter_by is None) or (len(filter_by) == 0)) else filter_by
        )
        filter_str = (
            None
