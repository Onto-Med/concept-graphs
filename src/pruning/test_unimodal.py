...
class Test_MLF(unittest.TestCase):
    ...
    def test_convert_to_graph(self):
        ...
        assert_edgelist_equal_graph(
            self, edgelist, G, edge_attribute="weight", edge_attribute_index=2
        )
    ...
    def test_fit_transform(self):
        ...
        assert_edgelist_equal_graph(
            ...
            edgelist_tr,
            g_tr,
            edge_attribute="significance",
            edge_attribute_index=3,
        )
    ...
    def test_check_types(self):
        ...
        # when g is a list
        edgelist = [
            (edge[0], edge[1], np.random.randint(0, 20)) for edge in g.get_edgelist()
        ]
        try:
...
def assert_df_equal_graph(testcase, df, graph, edge_attribute):
    ...
    testcase.assertEqual(
        len(set(df["source"].to_list() + df["target"].to_list())), G.vcount()
    )
...
def assert_edgelist_equal_graph(
    testcase, edgelist, graph, edge_attribute, edge_attribute_index
):
    """
...
def assert_df_edgelists_equal(
    testcase, df1, df2, columns=["source", "target", "weight", "significance"]
):
