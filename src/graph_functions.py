...
class GraphIncorp:
    ...
    @staticmethod
    def _update_documents_in_graph(graph: nx.Graph, phrase_id: int, documents: list[dict]):
        _docs = nx.get_node_attributes(graph, "documents").get(phrase_id, [])
        _present_doc_ids = set(d.get("id") for d in _docs if d.get("id", False))
        if any(_inter := _present_doc_ids.intersection(d.get("id") for d in documents if d.get("id", False))):
            ...
            logging.warning(f"Somehow a phrase tries to update one or more already existing document/s:"
    ...
    @staticmethod
    def _adding_phrase_to_graph(graph: nx.Graph, phrase_id: int, documents: list, label: str):
    ...
    def incorporate_phrase(self, phrase: dict, from_within: bool = False) -> Union[bool, 'GraphIncorp']:
        ...
        if graph_id is None or phrase_id is None:
            ...
            if not from_within:
                ...
                self._last_result = [(phrase_id, False,)]
                return self
        ...
        if not from_within:
            ...
            self._last_result = [(phrase_id, True,)]
            return self
