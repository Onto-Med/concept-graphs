...
from src.core.clustering import PhraseClusterFactory
...
class FactoryLoader:
    ...
    def with_active_objects(
        ...
        def _load(_step):
            try:
                active_objects[process][_step] = cls.load(
                    ...
                    emb_obj=active_objects.get(process, {}).get(
        ...
        if specific_step is None:
            for step in StepsName.ALL:
                if (step == StepsName.INTEGRATION) or active_objects.get(
                    process, {}
                ).get(step, False):
                    continue
                _load(step)
            ...
            if specific_step not in active_objects.get(process, {}):
                _load(specific_step)
        return (
            active_objects.get(process, {}).get(specific_step, None)
            if specific_step is not None
            else active_objects.get(process, {})
        )

    @classmethod
    def load_data(cls, path: str, process: str) -> DataProcessingFactory.DataProcessing:
    ...
    def load_embedding(
        ...
        if isinstance(sent_emb, dict):
            ...
            sent_emb.data_processing_obj = (
                cls.load_data(path, process) if data_obj is None else data_obj
            )
        if not hasattr(sent_emb, "sentence_embeddings"):
            raise AttributeError(
                "The provided object is not a SentenceEmbeddings object."
            )
        else:
            if sent_emb.sentence_embeddings is None:
                raise AttributeError(
                    "This SentenceEmbeddings object has no embeddings."
                )
        return sent_emb
    ...
    def load_clustering(
        ...
        if emb_obj is not None:
            if not hasattr(emb_obj, "sentence_embeddings"):
                logging.warning(
                    "The provided emb_obj seems not to be a SentenceEmbedding object. Trying to load it instead."
                )
                emb_obj = None
        phrase = PhraseClusterFactory.load(
            cluster_obj_path=Path(Path(path) / f"{process}_clustering.pickle"),
            embedding_obj=(
                cls.load_embedding(path, process, data_obj)
                if emb_obj is None
                else emb_obj
            ),
        )
    ...
    @classmethod
    def load_graph(cls, path: str, process: str) -> list[nx.Graph]:
        graphs = load_pickle(Path(Path(path) / f"{process}_graph.pickle"))
        if not isinstance(graphs, list):
            raise AttributeError(
                "The provided object doesn't seem to be a list of graphs."
            )
        else:
    ...
    @classmethod
    def load_integration(cls, path: str, process: str):
