...
def get_rag_status():
    ...
    if (
        ...
        and _has_rag
        and main_objects.active_rag.process == _process
    ):
        return jsonify(
            active=main_objects.active_rag.ready, name=_process, error=None
        ), int(HTTPResponses.OK)
    _err_string = (
    ...
    return jsonify(active=False, name=_process, error=_err_string), int(
        HTTPResponses.NOT_FOUND
    )
